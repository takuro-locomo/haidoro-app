...
import { ArrowDown, ArrowRight, ArrowUp, CircleCheck, Droplets, Info, Layers3, MessageCircle, Pause, Play, RotateCcw, Stethoscope } from "lucide-react";
        ...
        <Tabs value={regionId} onValueChange={selectRegion} className="region-tabs">
          ...
          <TabsContent value={regionId} className="region-content">
            ...
            <div className="explorer-grid" data-view={view}>
              <div id="anatomy-view" className="anatomy-view"><AnatomyAtlas key={regionId} region={region} target={target} /><Button className="next-view-button" onClick={() => { changeView("layers"); showResult(); }}><span><strong>注射の変化を見る</strong><small>このボタンを押してください</small></span><ArrowRight aria-hidden="true" /></Button></div>
              <section id="layer-view" className="layer-panel" aria-labelledby="layer-title">
                ...
                <div className="animation-controls"><div className="play-callout"><p id="play-instruction" className="play-instruction"><ArrowDown size={20} aria-hidden="true" />{playing ? "青い液体の広がりに注目してください" : progress === 100 ? "ボタンを押すと、もう一度見られます" : "下のボタンを押して、変化を見てみましょう"}</p><Button size="lg" className="play-button" onClick={play} aria-describedby="play-instruction">{playing ? <Pause /> : progress === 100 ? <RotateCcw /> : <Play />}{playing ? "一時停止" : progress === 100 ? "もう一度見る" : progress > 0 ? "続きを見る" : "注射の変化を見る"}</Button></div><div className="stage-controls" aria-label="表示する段階">{stages.map((s,i) => <Button key={s.phase} variant="ghost" onClick={() => seek(s.value)} aria-pressed={frame.phase === s.phase} className={frame.phase === s.phase ? "active-stage" : ""}><span>{i+1}</span>{s.name}</Button>)}</div></div>
            ...
            <div className="other-regions"><p>ほかにも気になるところはありませんか？</p><Button asChild size="lg" className="other-regions-button"><a href="#explore" onClick={stop}>他の部位も見る<ArrowUp aria-hidden="true" /></a></Button></div>
